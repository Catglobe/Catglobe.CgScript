...
public sealed class CgScriptWrapperGenerator : IIncrementalGenerator
{
   ...
   public void Initialize(IncrementalGeneratorInitializationContext context)
   {
      ...
      context.RegisterSourceOutput(combined, static (spc, pair) =>
      {
         ...
         // Emit a partial [CgScriptSerializer] declaration that registers the Params type.
         // This is required so that ctx.{ParamsClass} resolves at compile time and so that
         // JsonContent.Create(param, jsonTypeInfo: paramsInfo) serializes without going through
         // the TypeInfoResolver (which would fail on Linux/trimmed builds).
         var scriptNameLastSlash = scriptName.LastIndexOf('/');
         var paramsMethodName    = WrapperEmitter.ToPascalCase(scriptNameLastSlash >= 0
                                      ? scriptName.Substring(scriptNameLastSlash + 1)
                                      : scriptName);
         var paramsClass         = paramsMethodName + "Params";
         var contextPartial      = EmitSerializerPartial(serCtx.FullName, namespaceName, paramsClass);
         spc.AddSource($"{hintName}_JsonContext.g.cs",
            SourceText.From(contextPartial, Encoding.UTF8));
      });
   }

   private static string EmitSerializerPartial(string contextFullName, string wrapperNamespace, string paramsClass)
   {
      var lastDot  = contextFullName.LastIndexOf('.');
      var ctxNs    = lastDot >= 0 ? contextFullName.Substring(0, lastDot) : null;
      var ctxClass = lastDot >= 0 ? contextFullName.Substring(lastDot + 1) : contextFullName;
      ...
      if (ctxNs != null)
      {
         sb.AppendLine($"namespace {ctxNs}");
         sb.AppendLine("{");
      }
      sb.AppendLine($"    [JsonSerializable(typeof({wrapperNamespace}.CgScriptExtensions.{paramsClass}))]");
      sb.AppendLine($"    partial class {ctxClass} {{ }}");
      if (ctxNs != null)
         sb.AppendLine("}");
      return sb.ToString();
   }
